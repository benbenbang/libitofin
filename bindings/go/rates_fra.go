...
type Position int32
const ( PositionLong Position = iota; PositionShort )
...
func(s *Session) NewForwardRateAgreement(a FRAConfig)(*ForwardRateAgreement,error){
    ...
    err:=s.invoke(func()error{
        ...
        if a.MaturityDate!=nil{c.maturity_date=C.int32_t(a.MaturityDate.Serial())};if a.Discount!=nil{c.discount=C.uint64_t(a.Discount.id)}
        var e C.ItofinError;return ffiError(C.itofin_fra_new(s.ctx,c,&id,&e),&e)
    });if err!=nil{return nil,err};return &ForwardRateAgreement{object{s,uint64(id)}},nil
}
func(f *ForwardRateAgreement) value(field int32)(float64,error){
    var out C.double;s:=f.session;err:=s.invoke(func()error{
