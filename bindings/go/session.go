...
type Error struct { Code int32; Message string }
...
type request struct { fn func() error; done chan error }
...
func NewSession() (*Session,error) {
	...
	go func() {
		...
		var e C.ItofinError
		if C.itofin_abi_version() != 1 { ready <- errors.New("itofin: incompatible native ABI version"); return }
		err := ffiError(C.itofin_context_new(&s.ctx,&e),&e)
		ready <- err
		if err != nil { return }
		for r := range s.queue { r.done <- r.fn() }
	}()
	if err := <-ready; err != nil { <-s.stopped; return nil,err }
	return s,nil
}

func (s *Session) invoke(fn func() error) error {
	if s == nil { return errNilArgument("session") }
	s.gate.RLock()
	defer s.gate.RUnlock()
	if s.closed || s.queue == nil { return ErrClosed }
...
type object struct { session *Session; id uint64 }
func sameSession(s *Session, objects ...object) error {
	if s == nil { return errNilArgument("session") }
	for _,o := range objects {
		if o.session == nil || o.id == 0 { return errors.New("itofin: uninitialized object") }
		if o.session != s { return ErrSessionMismatch }
	}
	return nil
}
...
func (o object) Close() error {
	if o.session == nil || o.id == 0 { return nil }
	err := o.session.invoke(func() error {
		if o.session.released[o.id] { return nil }
		var e C.ItofinError
		err := ffiError(C.itofin_handle_release(o.session.ctx,C.uint64_t(o.id),&e),&e)
		if err == nil { o.session.released[o.id] = true }
		return err
	})
	if errors.Is(err,ErrClosed) { return nil }
	return err
}
