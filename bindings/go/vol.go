...
func (s *Session) BlackConstantVol(cfg BlackConstantVolConfig) (*BlackVolTermStructure, error) {
    if cfg.DayCounter == nil { return nil, fmt.Errorf("day counter is required") }
    deps := []object{cfg.DayCounter.object}
    var cal uint64
    if cfg.Calendar != nil { deps = append(deps, cfg.Calendar.object); cal = cfg.Calendar.id }
...
func (s *Session) BlackVarianceCurve(cfg BlackVarianceCurveConfig) (*BlackVolTermStructure, error) {
    if cfg.DayCounter == nil { return nil, fmt.Errorf("day counter is required") }
    if len(cfg.Dates) == 0 || len(cfg.Dates) != len(cfg.Volatilities) { return nil, fmt.Errorf("dates and volatilities must have equal nonzero lengths") }
    if err := sameSession(s, cfg.DayCounter.object); err != nil { return nil, err }
    dates := make([]C.int32_t, len(cfg.Dates)); vols := make([]C.double, len(cfg.Volatilities))
    for i := range dates { dates[i] = C.int32_t(cfg.Dates[i].serial); vols[i] = C.double(cfg.Volatilities[i]) }
    var monotone C.int32_t
    if cfg.ForceMonotoneVariance { monotone = 1 }
    var id C.uint64_t
    err := s.invoke(func() error {
        var e C.ItofinError
...
func (s *Session) BlackVarianceSurface(cfg BlackVarianceSurfaceConfig) (*BlackVolTermStructure, error) {
    if cfg.DayCounter == nil { return nil, fmt.Errorf("day counter is required") }
    if len(cfg.Dates) == 0 || len(cfg.Strikes) == 0 || len(cfg.Volatilities) != len(cfg.Strikes) { return nil, fmt.Errorf("invalid volatility matrix dimensions") }
    deps := []object{cfg.DayCounter.object}; var cal uint64
    if cfg.Calendar != nil { deps = append(deps, cfg.Calendar.object); cal = cfg.Calendar.id }
    if err := sameSession(s, deps...); err != nil { return nil, err }
    dates := make([]C.int32_t, len(cfg.Dates)); strikes := make([]C.double, len(cfg.Strikes)); var vols []C.double
    for i, d := range cfg.Dates { dates[i] = C.int32_t(d.serial) }
    for i, row := range cfg.Volatilities {
        if len(row) != len(dates) { return nil, fmt.Errorf("volatility matrix is ragged") }
        strikes[i] = C.double(cfg.Strikes[i])
        for _, v := range row { vols = append(vols, C.double(v)) }
    }
    var id C.uint64_t
    err := s.invoke(func() error {
        var e C.ItofinError
...
func (v *BlackVolTermStructure) BlackVol(t, strike float64, extrapolate bool) (float64, error) { return v.query(0, t, 0, strike, extrapolate) }
func (v *BlackVolTermStructure) BlackVariance(t, strike float64, extrapolate bool) (float64, error) { return v.query(1, t, 0, strike, extrapolate) }
func (v *BlackVolTermStructure) BlackForwardVol(t1, t2, strike float64, extrapolate bool) (float64, error) { return v.query(2, t1, t2, strike, extrapolate) }
func (v *BlackVolTermStructure) BlackForwardVariance(t1, t2, strike float64, extrapolate bool) (float64, error) { return v.query(3, t1, t2, strike, extrapolate) }
...
func (v *BlackVolTermStructure) BlackVolDate(d Date, strike float64, extrapolate bool) (float64, error) { return v.dateQuery(0, d, strike, extrapolate) }
func (v *BlackVolTermStructure) BlackVarianceDate(d Date, strike float64, extrapolate bool) (float64, error) { return v.dateQuery(1, d, strike, extrapolate) }
...
func (v *BlackVolTermStructure) MaxDate() (Date, error) { serial, err := v.control(0); return Date{serial: serial}, err }
func (v *BlackVolTermStructure) AllowsExtrapolation() (bool, error) { x, err := v.control(1); return x != 0, err }
