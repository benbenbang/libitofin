...
type SwapType int32
const ( SwapPayer SwapType = iota; SwapReceiver )
...
func (s *Session) NewVanillaSwap(a VanillaSwapConfig) (*VanillaSwap,error) {
    if a.FixedSchedule==nil || a.FloatingSchedule==nil || a.FixedDayCounter==nil || a.FloatingDayCounter==nil || a.Index==nil || a.Settings==nil {return nil,fmt.Errorf("swap arguments must not be nil")}
    var id C.uint64_t
    err:=s.invoke(func()error{
        if err:=sameSession(s,a.FixedSchedule.object,a.FloatingSchedule.object,a.FixedDayCounter.object,a.FloatingDayCounter.object,a.Index.object,a.Settings.object);err!=nil{return err}
        ...
        var e C.ItofinError;return ffiError(C.itofin_vanilla_swap_new(s.ctx,c,&id,&e),&e)
    });if err!=nil{return nil,err};return &VanillaSwap{object{s,uint64(id)}},nil
}
...
func (s *Session) MakeVanillaSwap(a MakeVanillaSwapConfig)(*VanillaSwap,error){
    if a.Index==nil||a.Settings==nil{return nil,fmt.Errorf("index and settings required")}
    var id C.uint64_t
    err:=s.invoke(func()error{
        objects:=[]object{a.Index.object,a.Settings.object};if a.FixedLegDayCounter!=nil{objects=append(objects,a.FixedLegDayCounter.object)}
        ...
        if a.FixedRate!=nil{c.flags|=1;c.fixed_rate=C.double(*a.FixedRate)}
        if a.EffectiveDate!=nil{c.flags|=2;c.effective_date=C.int32_t(a.EffectiveDate.Serial())}
        if a.Nominal!=nil{c.flags|=4;c.nominal=C.double(*a.Nominal)}
        if a.FixedLegTenor!=nil{c.flags|=8;c.fixed_length=C.int32_t(a.FixedLegTenor.Length);c.fixed_unit=C.int32_t(a.FixedLegTenor.Unit)}
        if a.FixedLegDayCounter!=nil{c.flags|=16;c.fixed_day_counter=C.uint64_t(a.FixedLegDayCounter.id)}
        var e C.ItofinError;return ffiError(C.itofin_make_vanilla_swap(s.ctx,c,&id,&e),&e)
    });if err!=nil{return nil,err};return &VanillaSwap{object{s,uint64(id)}},nil
}
...
func (s *Session) MakeOis(a MakeOisConfig)(*OvernightIndexedSwap,error){
    ...
    var id C.uint64_t
    err:=s.invoke(func()error{
        objects:=[]object{a.Index.object,a.Settings.object};if a.Discount!=nil{objects=append(objects,a.Discount.object)}
        if err:=sameSession(s,objects...);err!=nil{return err}
        c:=C.ItofinMakeSwapConfig{tenor_length:C.int32_t(a.Tenor.Length),tenor_unit:C.int32_t(a.Tenor.Unit),forward_length:C.int32_t(a.ForwardStart.Length),forward_unit:C.int32_t(a.ForwardStart.Unit),index:C.uint64_t(a.Index.id),settings:C.uint64_t(a.Settings.id)}
        if a.FixedRate!=nil{c.flags|=1;c.fixed_rate=C.double(*a.FixedRate)}
        if a.EffectiveDate!=nil{c.flags|=2;c.effective_date=C.int32_t(a.EffectiveDate.Serial())}
        if a.Nominal!=nil{c.flags|=4;c.nominal=C.double(*a.Nominal)}
        if a.PaymentLag!=nil{c.flags|=32;c.payment_lag=C.int32_t(*a.PaymentLag)}
        if a.Discount!=nil{c.flags|=64;c.discount=C.uint64_t(a.Discount.id)}
        if a.Averaging!=nil{c.flags|=128;c.averaging=C.int32_t(*a.Averaging)}
        var e C.ItofinError;return ffiError(C.itofin_make_ois(s.ctx,c,&id,&e),&e)
    });if err!=nil{return nil,err};return &OvernightIndexedSwap{object{s,uint64(id)}},nil
}
func (v *VanillaSwap) SetEngine(discount *YieldTermStructure,settings *Settings)error{
    if v==nil||discount==nil||settings==nil{return fmt.Errorf("swap, curve and settings required")}
    s:=v.session;return s.invoke(func()error{
        if err:=sameSession(s,v.object,discount.object,settings.object);err!=nil{return err}
...
func(v *VanillaSwap) IsCalculated()(bool,error){x,e:=swapValue(v.object,0,4);return x!=0,e}
...
func(v *OvernightIndexedSwap) IsCalculated()(bool,error){x,e:=swapValue(v.object,1,4);return x!=0,e}
