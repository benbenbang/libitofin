...
pub struct HestonHelperConfig {
    pub maturity_length: i32, pub maturity_unit: i32, pub calendar: u64,
    pub spot: f64, pub strike: f64, pub volatility: f64,
    pub risk_free_rate: f64, pub dividend_yield: f64, pub error_type: i32,
    pub reference_date: i32, pub day_counter: u64, pub settings: u64,
}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_heston_helper_new(ctx: *mut Context,
    cfg: HestonHelperConfig, out: *mut u64, error: *mut ItofinError) -> i32 {
    unsafe { with_context(ctx, error, |c| {
        check_ptr(out)?;
        ...
        let helper = HestonModelHelper::new(
            ...
            error_type(cfg.error_type)?, crate::settings_api::settings(c, cfg.settings)?);
...
pub struct SwaptionHelperConfig {
    pub maturity_length: i32, pub maturity_unit: i32,
    pub length: i32, pub length_unit: i32, pub volatility: f64, pub index: u64,
    pub fixed_tenor_length: i32, pub fixed_tenor_unit: i32,
    pub fixed_day_counter: u64, pub floating_day_counter: u64, pub curve: u64,
    pub error_type: i32, pub nominal: f64,
}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_swaption_helper_new(ctx: *mut Context,
    ...
    unsafe { with_context(ctx, error, |c| {
        ...
        let helper = SwaptionHelper::new(
            ...
            Period::new(cfg.fixed_tenor_length, crate::time_api::time_unit(cfg.fixed_tenor_unit)?),
            day_counter(c, cfg.fixed_day_counter)?, day_counter(c, cfg.floating_day_counter)?,
            c.get::<Handle<dyn YieldTermStructure>>(cfg.curve)?, error_type(cfg.error_type)?,
            None, cfg.nominal, libitofin::termstructures::volatility::VolatilityType::ShiftedLognormal,
            0.0, None, libitofin::cashflows::RateAveraging::Compound);
        output(out, c.insert(shared_mut(helper))?)
    }) }
}
/// Kind 0 Heston helper, 1 swaption helper.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_helper_error(ctx: *mut Context, helper: u64,
    kind: i32, out: *mut f64, error: *mut ItofinError) -> i32 {
    unsafe { with_context(ctx, error, |c| {
        check_ptr(out)?;
        let value = match kind {
            0 => c.get::<SharedMut<HestonModelHelper>>(helper)?.borrow_mut().calibration_error()?,
            1 => c.get::<SharedMut<SwaptionHelper>>(helper)?.borrow_mut().calibration_error()?,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_model_calibrate(ctx: *mut Context, model: u64,
    kind: i32, helpers: *const u64, helpers_len: usize, method: u64, criteria: u64,
    integration_order: usize, fix_reversion: i32, error: *mut ItofinError) -> i32 {
    unsafe { with_context(ctx, error, |c| {
        let ids = input_slice(helpers, helpers_len)?;
        if ids.is_empty() { return Err(BindingError::invalid("calibration helpers must not be empty")); }
        ...
        match kind {
            0 => {
                ...
                let helpers = ids.iter().map(|id| c.get::<SharedMut<HestonModelHelper>>(*id))
                    .collect::<BindingResult<Vec<_>>>()?;
                let engine = shared_mut(AnalyticHestonEngine::new(model.clone(), integration_order)?)
                    as SharedMut<dyn PricingEngine>;
                for helper in &helpers { helper.borrow_mut().base_mut().set_pricing_engine(engine.clone()); }
                let helpers: Vec<SharedMut<dyn CalibrationHelper>> = helpers.into_iter()
                    .map(|h| h as SharedMut<dyn CalibrationHelper>).collect();
                calibrate(&model, &helpers, &mut *method.borrow_mut(), &criteria, None, vec![], vec![])?;
            }
            1 => {
                if !(0..=1).contains(&fix_reversion) { return Err(BindingError::invalid("fix_reversion must be 0 or 1")); }
                let model = c.get::<SharedMut<HullWhite>>(model)?;
                let helpers = ids.iter().map(|id| c.get::<SharedMut<SwaptionHelper>>(*id))
                    .collect::<BindingResult<Vec<_>>>()?;
                let engine = shared_mut(JamshidianSwaptionEngine::new(model.clone())) as SharedMut<dyn PricingEngine>;
                for helper in &helpers { helper.borrow_mut().base_mut().set_pricing_engine(engine.clone()); }
                let helpers: Vec<SharedMut<dyn CalibrationHelper>> = helpers.into_iter()
                    .map(|h| h as SharedMut<dyn CalibrationHelper>).collect();
                let fixed = if fix_reversion != 0 { vec![true, false] } else { vec![] };
                calibrate(&model, &helpers, &mut *method.borrow_mut(), &criteria, None, vec![], fixed)?;
            }
