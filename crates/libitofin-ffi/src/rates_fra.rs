...
mod tests {
    ...
    fn par_forward_rate_amount_and_npv_match_the_closed_form() {
        let mut c=Context::new();let today=Date::new(15,Month::June,2026);
        let settings=shared(Settings::new());settings.set_evaluation_date(today);
        let flat=|rate|Handle::new(shared(FlatForward::with_rate(today,rate,Actual360::new(),Compounding::Continuous,Frequency::Annual)) as Shared<dyn YieldTermStructure>);
        let index=c.insert(shared(Euribor::three_months(flat(0.04),settings))).unwrap();let discount=c.insert(flat(0.06)).unwrap();
        let start=Date::new(17,Month::August,2026);let end=Date::new(17,Month::November,2026);let dc=Actual360::new();
        let t=dc.year_fraction(start,end);let f=((0.04*t).exp()-1.)/t;let amount=100.*(f-0.02)*t/(1.+f*t);
        for (position,sign) in [(0,1.),(1,-1.)] {
            let a=ItofinFraConfig {index,value_date:start.serial_number(),maturity_date:end.serial_number(),position,strike:0.02,notional:100.,discount};
            let mut id=0;unsafe {assert_eq!(itofin_fra_new(&mut c,a,&mut id,null_mut()),0);}
            for (field,expected) in [(0,sign*amount*(-0.06*dc.year_fraction(today,start)).exp()),(1,f),(2,sign*amount)] {
                let mut out=0.;unsafe {assert_eq!(itofin_fra_value(&mut c,id,field,&mut out,null_mut()),0);}
                assert!((out-expected).abs()<1e-12,"{out} vs {expected}");
            }
            let mut serial=0;unsafe{assert_eq!(itofin_fra_date(&mut c,id,1,&mut serial,null_mut()),0);}assert_eq!(serial,end.serial_number());
        }
        let invalid=ItofinFraConfig {index,value_date:start.serial_number(),maturity_date:end.serial_number(),position:99,strike:0.02,notional:100.,discount};
        let mut id=0;unsafe{assert_eq!(itofin_fra_new(&mut c,invalid,&mut id,null_mut()),INVALID_ARGUMENT);}
    }
}
