...
use libitofin::termstructures::volatility::{BlackConstantVol, BlackVarianceCurve,
    BlackVarianceSurface, BlackVolTermStructure, BlackVolTimeExtrapolation};
...
fn calendar(c: &Context, id: u64) -> BindingResult<Option<Calendar>> {
    if id == 0 { Ok(None) } else { c.get(id).map(Some) }
}
...
pub unsafe extern "C" fn itofin_black_constant_vol_new(ctx: *mut Context,
    ...
    unsafe { with_context(ctx, error, |c| {
        check_ptr(out)?;
        if !volatility.is_finite() || volatility < 0.0 { return Err(BindingError::invalid("volatility must be finite and nonnegative")); }
        let v = shared(BlackConstantVol::new(date(reference_date)?, calendar(c, cal)?,
            volatility, day_counter(c, dc)?)) as Shared<dyn BlackVolTermStructure>;
...
pub unsafe extern "C" fn itofin_black_variance_curve_new(ctx: *mut Context,
    ...
    dc: u64, monotone: i32, extrapolation: i32, out: *mut u64,
    error: *mut ItofinError) -> i32 {
    unsafe { with_context(ctx, error, |c| {
        check_ptr(out)?;
        let dates = input_slice(dates, count)?.iter().copied().map(date).collect::<BindingResult<Vec<_>>>()?;
        let vols = input_slice(vols, count)?;
        if vols.iter().any(|v| !v.is_finite() || *v < 0.0) { return Err(BindingError::invalid("volatilities must be finite and nonnegative")); }
        ...
        let v = shared(BlackVarianceCurve::with_interpolator(date(reference_date)?,
            &dates, vols, day_counter(c, dc)?, flag(monotone)?, extrapolation, Linear)?)
...
pub unsafe extern "C" fn itofin_black_variance_surface_new(ctx: *mut Context,
    ...
    strike_count: usize, vols: *const f64, vol_count: usize, dc: u64, cal: u64,
    out: *mut u64, error: *mut ItofinError) -> i32 {
    unsafe { with_context(ctx, error, |c| {
        check_ptr(out)?;
        if date_count == 0 || strike_count == 0 || date_count.checked_mul(strike_count) != Some(vol_count) {
            return Err(BindingError::invalid("volatility matrix must have strike_count rows and date_count columns"));
        }
        let dates = input_slice(dates, date_count)?.iter().copied().map(date).collect::<BindingResult<Vec<_>>>()?;
        let strikes = input_slice(strikes, strike_count)?.to_vec();
        let vols = input_slice(vols, vol_count)?;
        if strikes.iter().any(|x| !x.is_finite()) || vols.iter().any(|v| !v.is_finite() || *v < 0.0) {
            return Err(BindingError::invalid("strikes must be finite and volatilities finite and nonnegative"));
        }
        let mut matrix = Matrix::with_size(strike_count, date_count);
        for i in 0..strike_count { for j in 0..date_count { matrix[(i,j)] = vols[i * date_count + j]; } }
        let v = shared(BlackVarianceSurface::new(date(reference_date)?, calendar(c, cal)?,
            &dates, strikes, &matrix, day_counter(c, dc)?)?) as Shared<dyn BlackVolTermStructure>;
        output(out, c.insert(Handle::new(v))?)
    }) }
}
...
pub unsafe extern "C" fn itofin_black_vol_query(ctx: *mut Context, id: u64,
    ...
    unsafe { with_context(ctx, error, |c| {
        ...
        if (0..=3).contains(&kind) && (!t1.is_finite() || !t2.is_finite() || !strike.is_finite()) {
