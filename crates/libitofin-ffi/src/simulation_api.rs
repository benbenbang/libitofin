...
use crate::boundary::{BindingError, BindingResult, ItofinError, check_ptr, input_slice, without_context};
...
unsafe fn copy_output(out: *mut Real, capacity: usize, values: &[Real]) -> BindingResult<()> {
    if capacity < values.len() { return Err(BindingError::invalid("output capacity too small")); }
    if !values.is_empty() {
        check_ptr(out)?;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_gaussian_draws(count: usize, seed: u32, out: *mut Real, capacity: usize, error: *mut ItofinError) -> i32 {
    unsafe { without_context(error, || {
        if capacity < count { return Err(BindingError::invalid("output capacity too small")); }
        if count > 0 { check_ptr(out)?; }
        copy_output(out, capacity, &simulation_kernel::gaussian_draws(count,seed)?)
    }) }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn itofin_gbm_paths(input: *const ItofinGbmInput, out: *mut Real, capacity: usize, error: *mut ItofinError) -> i32 {
    unsafe { without_context(error, || {
        check_ptr(input)?;
        let i = &*input;
        if i.terminal_only != 0 && i.terminal_only != 1 { return Err(BindingError::invalid("invalid terminal_only flag")); }
        let correlation_len = i.assets.checked_mul(i.assets).ok_or_else(|| BindingError::invalid("correlation size overflow"))?;
        let request = GbmRequest {
            ...
            horizon: i.horizon, steps:i.steps, paths:i.paths, seed:i.seed,
            terminal_only:i.terminal_only == 1,
        };
        let length = simulation_kernel::output_len(&request)?;
        if capacity < length { return Err(BindingError::invalid("output capacity too small")); }
        check_ptr(out)?;
