//! Facade for the Monte Carlo European pricing engine: [`PyMCEuropeanEngine`].
//!
//! The core engine is generic over its RNG policy, which does not cross FFI
//! (D7), so the facade pins `PseudoRandom` - the policy that carries an error
//! estimate, and the one the core oracle prices against.
//!
...
use libitofin::pricingengines::vanilla::{MCEuropeanEngine, MakeMcEuropeanEngine};
